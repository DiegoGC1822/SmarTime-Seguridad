...
class RegistroUsuarioAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"mensaje": "Usuario creado"}, status=status.HTTP_201_CREATED
            )
...
class PasswordResetRequestAPIView(APIView):
    permission_classes = [
        AllowAny
    ]  # Cualquiera tiene acceso, ya que al no tener contraseña tiene que poder usarla, así no esté autenticado.
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if os.getenv("ENV") == "production":
                url = "https://smartime-production.up.railway.app/"
            else:
                url = "http://localhost:3000/"
            reset_link = f"{url}/{uidb64}/{token}/"
            ...
            )  # Envía el correo usando la configuración SMTP definida en settings.py

            return Response(
                {
                    "mensaje": "Se ha enviado un enlace para restablecer la contraseña al correo."
                },
                status=status.HTTP_200_OK,
            )
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    {"error": "UID inválido"}, status=status.HTTP_400_BAD_REQUEST
                )
